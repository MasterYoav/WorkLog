...
// Location guard for employer punches
...
export default function EmployerHome() {
  ...
  useEffect(() => {
    (async () => {
      const s = await loadShiftStart(employerNoStr);
      if (s) setShiftStartIso(s);
    })();
  }, [employerNoStr]);
  ...
  async function punch(type: 'in' | 'out') {
    try {
      ...
      if (type === 'in') {
        // שומר התחלה
        await saveShiftStart(employerNoStr, ts);
        setShiftStartIso(ts);

        // לוג כניסה
        await appendLocalPunch(employerNoStr, {
          kind: 'in',
          ts,
          lat: loc.lat,
          lng: loc.lng,
          acc: loc.accuracy ?? undefined,
          address: loc.addr,
        ...
        await appendLocalPunch(employerNoStr, {
          ...
          address: loc.addr,
          address_label: loc.addrLabel,
          started_at: shiftStartIso ?? undefined,
          duration_ms: dur,
        });

        await clearShiftStart(employerNoStr);
  ...
  const [meRows, setMeRows] = useState<
    Array<{ key: string; date: Date; durationMs: number; address_label: string }>
  >([]);

  useEffect(() => {
    (async () => {
      const pins = await loadLocalPunches(employerNoStr);
      const outs = pins
  ...
  const meFiltered = useMemo(
    () => meRows.filter((r) => r.date.getFullYear() === yMe && r.date.getMonth() === mMe),
    [meRows, yMe, mMe]
  );
  const meTotalMs = useMemo(
    () => meFiltered.reduce((s, r) => s + (r.durationMs || 0), 0),
    [meFiltered]
  );
  function shiftMonth(
    setM: (n: number) => void,
  ...
  // WORKERS (all-time totals from local punches)
  const [workers, setWorkers] = useState<Worker[]>([]);
  const [workerHours, setWorkerHours] = useState<Record<number, number>>({});
  useEffect(() => {
    (async () => {
      setWorkers(await listEmployerWorkers(employerNoStr));
    })();
  }, [employerNoStr]);
  ...
  const refreshProjects = async () => setProjects(await listProjects(employerNoStr));
  useEffect(() => {
    refreshProjects();
  }, [employerNoStr]);
  async function onCreateProject() {
    try {
      if (!projName.trim()) {
        Alert.alert('שגיאה', 'אנא הזן שם פרויקט');
        return;
      }
      if (!projLocation.trim()) {
        Alert.alert('שגיאה', 'אנא הזן מיקום');
        return;
      }
      const p = await createProject(employerNoStr, projName.trim(), projLocation.trim());
      setProjName('');
      setProjLocation('');
  ...
  function openProject(p: Project) {
    router.push({
      ...
      params: { employerNo: employerNoStr, projectId: String(p.id) },
    });
  }
  ...
  async function submitChangePassword() {
    try {
      if (!oldPass || !newPass || newPass.length < 4) {
        Alert.alert('שגיאה', 'מלא סיסמה נוכחית וסיסמה חדשה (לפחות 4 תווים).');
        return;
      }
      await changeEmployerPassword(employerNoStr, oldPass, newPass);
      setOldPass('');
      setNewPass('');
  ...
  function MenuButton({
    ...
    return (
      <TouchableOpacity onPress={onPress} style={{ paddingVertical: 10 }}>
        <Text
          style={{ color: active ? '#3b82f6' : (text.color as string), fontWeight: active ? '800' : '600' }}
        >
          {label}
        </Text>
      </TouchableOpacity>
    ...
    <SafeAreaView style={[{ flex: 1, flexDirection: 'row' }, bg]}>
      ...
      {menuOpen && (
        <View style={{ width: 240, padding: 14, borderRightWidth: 1, ...panel }}>
          ...
          <Text style={[{ fontSize: 18, fontWeight: '800', marginBottom: 8 }, text]}>
            {company || 'המעסיק שלי'}
          </Text>
          <Text style={[{ opacity: 0.7, marginBottom: 10 }, text]}>מס׳ מעסיק: {employerNoStr}</Text>
      ...
      <View style={{ flex: 1, paddingHorizontal: 16 }}>
        ...
        <View style={{ alignItems: 'flex-end', marginBottom: 6 }}>
          {!showChangePass ? (
            <TouchableOpacity onPress={() => setShowChangePass(true)}>
              <Text style={[{ textDecorationLine: 'underline' }, text]}>שינוי סיסמה</Text>
            </TouchableOpacity>
          ) : (
        ...
        {section === 'me' && (
          <FlatList
            ...
            ListEmptyComponent={
              <Text style={[{ opacity: 0.7, textAlign: 'center', marginTop: 16 }, text]}>
            ...
            ListHeaderComponent={
              <View>
                <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }}>
                  ...
                  {/* NEW: navigate to monthly summary screen */}
                  <TouchableOpacity
                    onPress={() =>
                      router.push({
                  ...
                  >
                    <Text style={{ color: '#fff', fontWeight: '800' }}>סיכום חודשי</Text>
                  </TouchableOpacity>
                </View>
              </View>
            }
              ...
              <View>
                ...
                <View style={{ marginTop: 10, padding: 12, borderWidth: 1, borderRadius: 10, gap: 8, ...panel }}>
                  ...
                  <TextInput
                    ...
                    placeholder="למשל: שיפוץ רח' הרצל 12"
                    placeholderTextColor={isDark ? '#aaa' : '#888'}
                    style={{
                      borderWidth: 1,
                      borderColor: isDark ? '#333' : '#ccc',
                  ...
                  <TextInput
                    ...
                    placeholder="Tel Aviv / Jerusalem / London"
                    placeholderTextColor={isDark ? '#aaa' : '#888'}
                    style={{
                      borderWidth: 1,
                      borderColor: isDark ? '#333' : '#ccc',
